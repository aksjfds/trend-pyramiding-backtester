...
- 只做多。
- 信号使用最新已确认收盘 K 线。实盘首笔开仓候选不会随新 K 自动生成；只有用户在网页点击“生成候选”时才扫描全部已选币种。随后用户选择候选开仓，worker 再次核验行情、预算、最小张数和止损后提交订单。
- 默认四档风险权重：30% / 30% / 20% / 20%。
- 首笔开仓支持两种人工入口：从策略生成的候选中确认，或直接指定当前策略已管理的空仓币种。指定币种不要求满足候选信号，但仍按最新已收盘 K 线计算策略初始止损和资金张数。首仓成交后，策略自动接管该币种；只给盈利仓加仓，价格相对上次成交继续上涨 0.75 ATR 后允许下一档加仓。
