...
- 只做多。
- 信号使用最新已确认收盘 K 线。实盘首笔开仓候选不会随新 K 自动生成；只有用户在网页点击“生成候选”时才扫描交易所当前全部受支持的 USDT 永续合约；符合条件的候选按 24 小时 USDT 成交量从高到低排列，并在列表中显示该数值；USDT 口径按 OKX 合约基础币成交量 × 最新价折算。随后用户选择候选开仓，worker 再次核验行情、预算、最小张数和止损后提交订单。
